...
\begin{itemize}
	...
	\item Whether the stack/queue is array-based (int fields where necessary, initial capacity of 100) or linked-list based (doubly-linked, head and tail pointer to dummy nodes).
	\item How many items have been inserted into the stack/queue ($n \leq 30$)
\end{itemize}

When answering, remember that integers are 8 bytes, chars are 1 byte, and references are 8 bytes. The size of objects (e.g., ListNode) is just the sum of the size of its fields. 
